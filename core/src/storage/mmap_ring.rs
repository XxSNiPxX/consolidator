...
/// Default meta size used when user doesn't provide one (1 MiB)
pub const DEFAULT_META_SIZE: usize = 1024 * 1024; // 1 MiB
...
impl MmapRing {
    ...
    pub fn open(cfg: &MmapRingConfig) -> Result<Self> {
        ...
        let computed_meta = MmapRing::compute_meta_size(cfg.index_slots)?;
        let meta_size = if cfg.meta_size == 0 {
            computed_meta
        } else {
            cfg.meta_size
        };
